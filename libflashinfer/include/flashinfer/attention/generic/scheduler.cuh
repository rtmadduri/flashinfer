...
inline auto PrefillBinarySearchKVChunkSize(const bool enable_gpu_graph,
  ...
  return std::make_tuple(enable_gpu_graph || low < max_kv_len, low);
}
...
inline gpuError_t BatchDecodeWithPagedKVCacheWorkEstimationDispatched(
    ...
    bool enable_gpu_graph, gpuStream_t stream) {
  ...
  DISPATCH_COMPUTE_CAP_DECODE_NUM_STAGES_SMEM(compute_capacity, NUM_STAGES_SMEM, {
    ...
    if (batch_size * gdy >= max_grid_size) {
      ...
      if (new_batch_size == batch_size && !enable_gpu_graph) {
...
inline gpuError_t BatchDecodeWithPagedKVCacheWorkEstimationDispatchedMlaCuteSM80(
  ...
  // FI_GPU_CALL(gpuOccupancyMaxActiveBlocksPerMultiprocessor(&num_blocks_per_sm, kernel,
  //                                   num_threads, smem_size));
  // fixme: num_blocks_per_sm is 0 derived from gpuOccupancyMaxActiveBlocksPerMultiprocessor at
...
inline gpuError_t DecodePlan(void* float_buffer, size_t float_workspace_size_in_bytes,
                             ...
                             uint32_t num_qo_heads, uint32_t page_size, bool enable_gpu_graph,
  ...
  FI_GPU_CALL(work_estimation_func(split_kv, max_grid_size, kv_chunk_size_in_pages, new_batch_size,
                                   ...
                                   enable_gpu_graph, stream));
  size_t padded_batch_size;
  plan_info.enable_gpu_graph = enable_gpu_graph;
  plan_info.split_kv = split_kv;
  padded_batch_size =
      (enable_gpu_graph) ? (split_kv ? max_grid_size / gdy : batch_size) : new_batch_size;
...
inline auto PrefillSplitQOKVIndptr(IdType* qo_indptr_h, IdType* kv_indptr_h,
                                   ...
                                   bool enable_gpu_graph, int32_t window_left,
  ...
  if (disable_split_kv) {
    ...
    std::tie(split_kv, kv_chunk_size) =
        PrefillBinarySearchKVChunkSize(enable_gpu_graph, max_batch_size_if_split, packed_qo_len_arr,
  ...
  const size_t padded_batch_size =
      enable_gpu_graph ? std::max(max_batch_size_if_split, total_num_tiles_q) : new_batch_size;
  FLASHINFER_CHECK(new_batch_size <= padded_batch_size,
                   ...
                   "split size, please consider disabling gpu graph.");
...
inline gpuError_t PrefillPlan(void* float_buffer, size_t float_workspace_size_in_bytes,
                              ...
                              bool enable_gpu_graph, uint32_t sizeof_dtype_o, int32_t window_left,
                              int32_t fixed_split_size, bool disable_split_kv, gpuStream_t stream) {
  ...
  auto [split_kv, new_batch_size, padded_batch_size, cta_tile_q, kv_chunk_size, request_indices_vec,
      ...
      PrefillSplitQOKVIndptr(qo_indptr_h, kv_indptr_h, total_num_rows, batch_size, num_qo_heads,
                             ...
                             enable_gpu_graph, window_left, fixed_split_size, disable_split_kv);
...
inline gpuError_t PrefillSM90Plan(
    ...
    bool enable_gpu_graph, uint32_t sizeof_dtype_o, gpuStream_t stream) {
  if (num_qo_heads % num_kv_heads != 0) {
    std::ostringstream err_msg;
    err_msg << "num_qo_heads " << num_qo_heads << " should be divisible by num_kv_heads "
            << num_kv_heads;
...
template <typename IdType>
inline gpuError_t TwoStageHolisticPlan(void* float_buffer, size_t float_workspace_size_in_bytes,
                                       void* int_buffer, void* page_locked_int_buffer,
